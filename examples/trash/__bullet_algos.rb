...
module BulletNeraiutidan
  def compute
    super
    if @win.press_button?(SDL2::Key::K1) || (joy && joy.button(0))
...
module BulletTuibi
  def compute
    super
    if @win.press_button?(SDL2::Key::K2) || (joy && joy.button(1))
...
module Bullet3waydan
  def compute
    super
    if @win.press_button?(SDL2::Key::K4) || (joy && joy.button(2))
...
module BulletRandom
  def compute
    super
    if @win.press_button?(SDL2::Key::K3) || (joy && joy.button(3))
...
module BulletUzumaki
  ...
  def compute
    super
    if @win.press_button?(SDL2::Key::K5) || (joy && joy.button(4))
...
module BulletUzumakix2
  def compute
    super
    if @win.press_button?(SDL2::Key::K6) || (joy && joy.button(5))
      div, mod = @frame_counter.divmod(60 / 10)
      if mod.zero?
        uzumaki2(div, +1)
        uzumaki2(div, -1)
...
module Bullet128waydan
  def compute
    super
    if @win.press_button?(SDL2::Key::K7) || (joy && joy.button(6))
...
module BulletUzumakix2Fast
  def compute
    super
    if @win.press_button?(SDL2::Key::K8) || (joy && joy.button(7))
      div, mod = @frame_counter.divmod(60 / 20)
      if mod.zero?
        uzumaki2fast(div, +1)
        uzumaki2fast(div, -1)
