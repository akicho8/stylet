...
class GunShip
  ...
  def update
    ...
    if @joystick_index
      if joy = frame.joys[@joystick_index]
        update_by_joy(joy)
      end
    end
    ...
    if dir = axis_angle
      ...
      if Stylet::CollisionSupport.rect_in?(frame.rect, next_pos)
        @pos = next_pos
      end
    end

    frame.draw_triangle(@pos, :radius => @size, :angle => @pos.angle_to(@target.pos))
  end
end
